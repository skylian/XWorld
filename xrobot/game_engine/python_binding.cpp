...
Thing::Thing() :
        label_("Nothing"), position_(vec2tuple(glm::vec3(0,0,0))) {}
...
void Thing::Sync() {
	if (auto robot_sptr = robot_.lock()) {
        glm::vec3 pos;
        glm::vec4 quat;
        robot_sptr->root_part_->pose(pos, quat);
		position_ = vec2tuple(pos);
        orientation_ = vec2tuple(quat);	
...
boost::python::list Playground::UpdateLidar(
...
void Playground::EnableNavigation(
...
NavAgent Playground::SpawnNavigationAgent(
...
void Playground::AssignNavigationAgentTarget(
        const NavAgent& agent,
        const boost::python::list& position) {
    if (crowd_) {
        if (agent.GetUid() < 0) { return; }
...
void Playground::LoadXWorldScene(const std::string& filename) {
    ...
    for (Json::ArrayIndex index = 0; index < json_levels->size(); index++) {
        ...
        if (!json_get_object(
                    json_positions, json_level, "Position", Json::objectValue)) {
            return;
        }
        ...
        if (!json_get_object(
                    json_scales, json_level, "Scale", Json::objectValue)) {
            return;
        }
        ...
        Thing obj = SpawnAnObject(
                path, position, orientation, uniform_scale, label, fixed);
        scene_dict[uid] = obj;
...
void Playground::LoadBasicObjects(
        const boost::python::list& doors,
...
void Playground::SpawnModelsConf(const boost::python::dict& conf) {
    ...
    if (conf.has_key("single")) {
        ...
        for (int i = 0; i < boost::python::len(val) / 3; ++i) {
            std::string path =
                    boost::python::extract<std::string>(val[3 * i + 0]);
    ...
    if (conf.has_key("stack")) {
        ...
        for (int i = 0; i < boost::python::len(val) / 5; ++i) {
            std::string path_0 =
                    boost::python::extract<std::string>(val[5 * i + 0]);
            std::string path_1 =
                    boost::python::extract<std::string>(val[5 * i + 1]);
...
void Playground::LoadModels(
        const boost::python::list& models, const boost::python::list& tags) {
    std::shared_ptr<MapGrid> scene_grid 
        = std::dynamic_pointer_cast<MapGrid>(scene_);
...
boost::python::list Playground::LocatePositionInGrid(
...
boost::python::list Playground::LoadSceneConfigure(
        const int w, const int l, const int n, const int d) {
    std::shared_ptr<MapGrid> scene_grid =
            std::dynamic_pointer_cast<MapGrid>(scene_);
...
void Playground::CreateEmptyScene(
        const float min_x,
        const float max_x,
        const float min_z,
        const float max_z) {
...
Thing Playground::SpawnAnObject(
    ...
    if (occupy) {
        ...
        if (auto scene_grid = std::dynamic_pointer_cast<MapGrid>(scene_)) {
            ...
            if (!scene_grid->resolve_path_) {
                SubTileSPtr subtile =scene_grid->GetSubTileFromWorldPosition(
...
void Playground::AttachCameraTo(
        const Thing& object, const boost::python::list& offset_py) {
	glm::vec3 offset = list2vec3(offset_py);
	main_camera_ = scene_->world_->add_camera(
            vec3(0, 0, 0), offset, camera_aspect_);
...
void Playground::FreeCamera(
        const boost::python::list& position, const float yaw, const float pitch) {
...
void Playground::UpdateFreeCamera(
        const boost::python::list& position, const float yaw, const float pitch) {
    glm::vec3 pos = list2vec3(position);
...
boost::python::object Playground::GetCameraRGBDRaw() {
    ...
    boost::python::object ret_val=
            boost::python::object(boost::python::handle<>(py_buf));
    return ret_val;
}
...
std::string Playground::Grasp() {
	...
	glm::vec3 from = main_camera_->position_;
	glm::vec3 to =
            main_camera_->front_ * bullet_engine::kDistanceThreshold + from;
...
void Playground::Use(const int object_id) {
    ...
    if (bullet_id >= 0) {
        ...
        if (object_id < inventory_->GetNumUsedSpace()) {
            std::string use_tag = boost::python::extract<std::string>(
                    current_objects_[object_id]);
            object->InteractWith(use_tag);
        }
    }
}

void Playground::ControlJointPositions(
        const Thing& object, 
    ...
    if (auto object_sptr = object.GetPtr().lock()) {
        ...
        for (int i = 0; i < boost::python::len(joints_position_keys); ++i) {
            int joint_id   =
                    boost::python::extract<int>(joints_position_keys[i]);
            float position =
                    boost::python::extract<float>(joint_positions[joint_id]);

            auto joint_ptr = object_sptr->joints_[joint_id];
            joint_ptr->SetJointMotorControlPosition(
                    position, 0.1f, 1.0f, max_force);
        }
    }
}

void Playground::ControlJointVelocities(
        const Thing& object, 
    ...
    if (auto object_sptr = object.GetPtr().lock()) {
        ...
        for (int i = 0; i < boost::python::len(joint_velocities_keys); ++i) {
            ...
            float velocity = boost::python::extract<float>(
                    joint_velocities[joint_id]);
...
bool Playground::QueryContact(const Thing& object) {
    if (auto object_sptr = object.GetPtr().lock()) {
        auto object_root_ptr = object_sptr->root_part_;

        std::vector<ContactPoint> contact_points;
        scene_->world_->GetContactPoints(
                object_sptr, object_root_ptr, contact_points);
...
boost::python::list Playground::QueryObjectNearObject(
        Thing& object, const bool exlude, const float dist) {
    ...
    for (auto& kv : bullet_world->id_to_robot_) {
        ...
        if (!body->is_recycled() && body->body_data_.body_uid >= 0) {
            ...
            glm::vec3 pos = body->root_part_->position();
...
bool Playground::QueryObjectAABBIntersect(
        const Thing& object_a, const Thing& object_b) {
...
bool Playground::QueryObjectWithLabelAtForward(const std::string& label) {
    ...
    for (int i = 0; i < temp.size(); ++i) {
    	...
    	if (glm::distance(camera_center, aabb_center) < 3.0f) {
            float ang = glm::dot(
                    glm::normalize(aabb_center - camera_center),
                    camera_front);
    		if (ang > 0.9f) {
    			return true;
            }
...
void Playground::Teleport(
	...
	if (auto object_sptr = object.GetPtr().lock()) {
        ...
        bullet_engine::set_pose(
                scene_->world_->client_,
                root_part_->body_uid_,
                position,
